...
void dramAccess(bool read, string word1, string word2, int lineN)
{
    ...
    if (row_buffer != row_no)
    {
        if (row_buffer >= 0)
        {
            fout << "cycle " << clockC + 1 << "-" << clockC + RAD << ": DRAM writeback row " << row_buffer << "\n";
            clockC = clockC + RAD;

            fout << "cycle " << clockC + 1 << "-" << clockC + RAD << ": DRAM activate row " << row_no << "\n";
            clockC = clockC + RAD;
    ...
    fout << "cycle " << clockC + 1 << "-" << clockC + CAD << ": DRAM column access; ";
    clockC = clockC + CAD;
...
{
    if (skippedLines->find(lineN) != skippedLines->end())
    {
...
int getRowNumber(string addr, int lineN)
{
    int a;
    if (addr.at(0) == '$')
        a = RegisterFile[getRegister(addr)];
    else if (addr.at(0) == '(')
    {
        string reg = addr.substr(1, addr.find(')') - 1);
        a = RegisterFile[getRegister(reg)];
    }
    else if (addr.at(0) >= 48 && addr.at(0) <= 57)
    {
        if (addr.find('$') != std::string::npos)
        {
            ...
            a = RegisterFile[getRegister(reg)] + stoi(offset);
        }
        else
        {
            a = stoi(addr);
...
void reorderingInstructions(string line, int lineN)
{
    ...
    {
        string f_word;
        istringstream l(AssemblyLines[i]);
        l >> f_word;
        if (f_word == "")
            continue;
        if (f_word == "sw" || f_word == "lw")
        {
            ...
            l >> word2;
            string x;
            if (depRegisters->find(word1.substr(0, word1.length() - 1)) != depRegisters->end())
            {
                return;
            }
            depRegisters->insert(word1.substr(0, word1.length() - 1));
            if (getRowNumber(word2, i) == row_buffer)
            {
